...
```
Network Attachment (소비자 측 리소스)
├── Region 지정
├── Subnet 할당
└── Authorization Policy (내장)
    ├── Connection Preference: ACCEPT_AUTOMATIC 또는 ACCEPT_MANUAL
    ├── Producer Accept List: [프로젝트 ID 목록]
    └── Producer Reject List: [프로젝트 ID 목록] (선택)
```
...
[Consumer VPC]                    [Producer VPC]
     |                                 |
  PSC Endpoint ──→ Service Attachment ──→ Internal LB ──→ Backend
  (내부 IP)         (Connection Policy)
```
...
[Producer VPC]                    [Consumer VPC]
     |                                 |
  PSC Interface ──→ Network Attachment ──→ Consumer Resources
                    (Authorization Policy)    (+ transitive 접근)
```
