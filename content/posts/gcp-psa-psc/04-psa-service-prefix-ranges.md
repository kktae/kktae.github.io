...
```mermaid
graph TD
    R["10.0.0.0/16<br/>(65,536 주소)"]
    R --> A["Cloud SQL (리전 A)<br/>/24 자동 할당"]
    R --> B["Cloud SQL (리전 B)<br/>/24 자동 할당"]
    R --> C["Memorystore Redis<br/>/29 ~ /28 자동 할당"]
    R --> D["Memorystore Memcached<br/>/26 자동 할당"]
    R --> E["Filestore<br/>/29 또는 /26 자동 할당"]
    R --> F["AlloyDB<br/>/24 리전별 자동 할당"]
```
