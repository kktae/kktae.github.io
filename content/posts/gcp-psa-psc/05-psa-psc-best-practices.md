...
```mermaid
flowchart TD
    Q1{서비스가 PSC를<br/>지원하는가?}
    Q1 -- No --> PSA[PSA 사용]
    Q1 -- Yes --> Q2{여러 VPC/프로젝트에서<br/>접근 필요?}
    Q2 -- Yes --> PSC1[PSC 사용]
    Q2 -- No --> Q3{세밀한 엔드포인트별<br/>접근 제어 필요?}
    Q3 -- Yes --> PSC2[PSC 사용]
    Q3 -- No --> Q4{VPC Peering<br/>제약이 문제인가?}
    Q4 -- Yes --> PSC3[PSC 사용]
    Q4 -- No --> Q5{PUPI IP 범위<br/>사용 필요?}
    Q5 -- Yes --> PSC4[PSC 사용]
    Q5 -- No --> Q6{기존 PSA 환경이<br/>안정적 + 단순 아키텍처?}
    Q6 -- Yes --> KEEP[PSA 유지 가능<br/>향후 PSC 전환 권장]
    Q6 -- No --> PSC5[PSC 사용]
```
